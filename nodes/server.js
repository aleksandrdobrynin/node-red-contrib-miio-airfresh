...
module.exports = function (RED) {
            ...
            return new Promise(function (resolve, reject) {
                node.miio = miio.device({
                    ...
                    node.device.on('thing:initialized', () => {
                        node.log('Miio AirFresh: Initialized');
                    });

                    node.device.on('thing:destroyed', () => {
                        node.log('Miio AirFresh: Destroyed');
                    });
                    ...
                    node.warn('Miio AirFresh Error: ' + err.message);
                    reject(err);
                });
            });
        }
            ...
            return new Promise(function (resolve, reject) {
                if (force || !that.status) {
                    if (that.device !== null && that.device !== undefined) {
                        that.device.loadProperties(["power", "mode", "favourite_speed", "control_speed", "pm25", "co2", "temperature_outside", "filter_rate", "filter_day", "ptc_status", "child_lock", "sound", "display"]).then(result => {
    ...
    RED.nodes.registerType('miio-airfresh-server', ServerNode, {});
};
